...
const drawWheel = (
  ...
  if (canvas?.getContext('2d')) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // WHEEL OUTER BORDER3
      ctx.strokeStyle = outerBorderWidth <= 0 ? 'transparent' : '#7e7e7e';
      ctx.lineWidth = outerBorderWidth + 25;
      ctx.beginPath();
      ctx.arc(
        centerX,
        centerY,
        outsideRadius - ctx.lineWidth - 0 / 2,
        0,
        2 * Math.PI
      );
      ctx.closePath();
      ...
      // WHEEL OUTER BORDER2
      ctx.strokeStyle = outerBorderWidth <= 0 ? 'transparent' : '#d3d3d3';
      ctx.lineWidth = outerBorderWidth + 20;
      ctx.beginPath();
      ctx.arc(
        centerX,
        centerY,
